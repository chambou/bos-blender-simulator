...
# --------------------------------------------------
config_path = PROJECT_ROOT / "config_stereo.json"
...
for k in range(0,config["BOS"]["cameras_number"]):
    ...
    x_cam = -(config["BOS"]["cameras_number"]-1) * config["BOS"]["cameras_spacing"] / 2 + k *config["BOS"]["cameras_spacing"]
    y_cam = 0
...
x_screen = 0
y_screen = config["BOS"]["distance_camera_screen"]
z_screen =  size_screen_height / 2 + config["BOS"]["height"]

# Plane
plane = create_plane(
    location=mathutils.Vector([x_screen, y_screen, z_screen]),
